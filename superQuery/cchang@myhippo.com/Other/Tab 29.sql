...
--       , is_ebsl
  ...
  , aggregated as (
    select 
        ...
        ,damage_description
        ,case 
        when strpos(loss_description,'structural') > 0 then 'yes'
        when strpos(loss_description,'structure') > 0 then 'yes'
        when strpos(loss_description,'foundation') > 0 then 'yes'
        when strpos(loss_description,'wall') > 0 then 'yes'
        when strpos(loss_description,'ceiling') > 0 then 'yes'
        when strpos(loss_description,'roof') > 0 then 'yes'
        when strpos(loss_description,'attic') > 0 then 'yes'
        when strpos(loss_description,'door') > 0 then 'yes'
        when strpos(loss_description,'fireplace') > 0 then 'yes'
        when strpos(loss_description,'chimney') > 0 then 'yes'
        when strpos(loss_description,'porch') > 0 then 'yes'
        when strpos(loss_description,'garage') > 0 then 'yes'
        
        when strpos(damage_description,'structural') > 0 then 'yes'
        when strpos(damage_description,'structure') > 0 then 'yes'
        when strpos(damage_description,'foundation') > 0 then 'yes'
        when strpos(damage_description,'wall') > 0 then 'yes'
        when strpos(damage_description,'ceiling') > 0 then 'yes'
        when strpos(damage_description,'roof') > 0 then 'yes'
        when strpos(damage_description,'attic') > 0 then 'yes'
        when strpos(damage_description,'door') > 0 then 'yes'
        when strpos(damage_description,'fireplace') > 0 then 'yes'
        when strpos(damage_description,'chimney') > 0 then 'yes'
        when strpos(damage_description,'porch') > 0 then 'yes'
        when strpos(damage_description,'garage') > 0 then 'yes'
        else 'no' end as relevant_claim
