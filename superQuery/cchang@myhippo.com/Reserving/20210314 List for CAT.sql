...
--       , is_ebsl
    ...
    , loss_description
    , damage_description
  FROM dbt_actuaries.ext_all_claims_combined_20210314 mon
  ...
  left join (select policy_id, calculated_fields_non_cat_risk_class, calculated_fields_cat_risk_class, renewal_number from dw_prod_extracts.ext_policy_snapshots where date_snapshot = '2021-03-14') eps on eps.policy_id = mon.policy_id
  ...
  left join dbt_actuaries.cat_coding_w_loss_20210314 cc on (case when tbl_source = 'topa_tpa_claims' then ltrim(mon.claim_number,'0') else mon.claim_number end) = cast(cc.claim_number as string)
  WHERE 1=1
  and date_knowledge = '2021-03-14'
...
--   where claim_number = 'HCA-3723298-00-01'
--   order by 1
