...
left join (select policy_id, renewal_number from dw_prod_extracts.ext_policy_snapshots where date_snapshot = '2020-07-31') eps on eps.policy_id = cd.policy_id
  WHERE date_knowledge = '2020-07-31'
  and carrier <> 'Canopius'
)
, claims as (
...
, round(sum(excess_non_cat_incurred) / sum(earned_prem_x_ebsl),3) as excess_NC
, round(sum(cat_incurred) / sum(earned_prem_x_ebsl),3) as cat
, round(sum(total_incurred) / sum(earned_prem_x_ebsl),3) as total
from combined
where 1=1
