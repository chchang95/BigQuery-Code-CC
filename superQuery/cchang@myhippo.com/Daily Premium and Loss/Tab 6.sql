...
, case when cast(calculated_fields_non_cat_risk_score as numeric) is null then cast(sf.risk_score as numeric) 
else cast(calculated_fields_non_cat_risk_score as numeric) end as calculated_fields_non_cat_risk_score
...
left join (select policy_id, risk_score from scoring_final) sf on sf.policy_id = eps.policy_id
