...
case when lag(total_paid) over(partition by claim_id order by date_knowledge) is null and total_paid > 0 then 1  
when total_paid <> lag(total_paid) over(partition by claim_id order by date_knowledge) then 1 else 0 end as incremental_count_claims_w_payment,

case when lag(total_incurred) over(partition by claim_id order by date_knowledge) is null and total_incurred > 0 then 1
when total_incurred <> lag(total_incurred) over(partition by claim_id order by date_knowledge) then 1 else 0 end as incremental_count_claims_w_reported,
...
, sum(case when ac.total_paid_change > 0 then 1 else 0 end) as incremental_count_claim_with_payment
, sum(case when ac.total_incurred_change > 0 then 1 else 0 end) as incremental_count_claim_with_incurred
