...
with top as(SELECT 
    ...
    s.renewal_number as orig_renewalnumber,     
    snap2.policy_number as policy_number,       
    snap2.renewal_number as renewal_number,         
    s.coverage_a as initial_cova,           
    dp1.channel,              
    dp1.state,                
    dp1.product,              
    dp1.tenure,               
    dp1.carrier,              
    s.coverage_a as written_coverage_a,
    s.written_total-s.written_optionals_equipment_breakdown - s.written_optionals_service_line as written_total_ex_ebsl,          
    s.written_total_optionals,      
    s.written_total,            
    s.status as original_policy_status,           
    snap2.status as renewal_policy_status,                
...
JOIN                
  dw_prod_extracts.ext_policy_snapshots s               
ON              
  s.policy_id = dp1.policy_id  and s.date_snapshot = current_date               
LEFT JOIN               
 dw_prod_extracts.ext_policy_snapshots snap2                
ON              
  snap2.policy_id = dp1.next_policy_id and snap2.date_snapshot = current_date               
LEFT JOIN               
  dw_prod.dim_policies dp2              
ON              
  dp1.next_policy_id = dp2.policy_id                
LEFT JOIN               
 dw_prod.dim_renewals ren               
ON              
 dp1.next_policy_id=ren.policy_id    
left join
    dw_prod.fct_policy_updates upd
on s.latest_policy_update_id = upd.policy_update_id
WHERE               
